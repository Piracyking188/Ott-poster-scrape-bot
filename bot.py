...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Prefer a vertical poster, then landscape, then cover art
    image_url = None
    if isinstance(data, dict):
        image_url = data.get("portrait") or data.get("landscape") or data.get("cover")
    ...
    if image_url:
        try:
            ...
            await update.message.reply_text(f"Got a poster URL but couldn't send it as an image: {image_url}")
            return
