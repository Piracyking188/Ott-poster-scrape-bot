...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if sent_anything:
        ...
        if logo:
            try:
                await update.message.reply_photo(photo=logo, caption="Logo")
            except Exception as exc:
                log.warning("reply_photo (logo) failed: %s", exc)

        thumbnails = data.get("thumbnails") if isinstance(data, dict) else None
        if isinstance(thumbnails, list):
            for i, thumb in enumerate(thumbnails[:5], start=1):  # cap at 5 to avoid spam
                if isinstance(thumb, str) and thumb:
                    try:
                        await update.message.reply_photo(photo=thumb, caption=f"Thumbnail {i}")
                    except Exception as exc:
                        log.warning("reply_photo (thumbnail) failed: %s", exc)
        return
